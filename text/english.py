...
from text.symbols import punctuation, SYMBOLS_TABLE
...
def replace_phs(phs):
    ...
    phs_new = []
    for ph in phs:
        if ph in SYMBOLS_TABLE:
            phs_new.append(ph)
        elif ph in rep_map.keys():
            phs_new.append(rep_map[ph])
        else:
            print(f"ph not in SYMBOLS_TABLE: <{ph}>")
    return phs_new
...
def text_normalize(text):
    ...
    for p, r in rep_map.items():
        text = re.sub(p, r, text)   # replace chinese punctuation to english punctuation

    text = str(text)
    text = normalize_numbers(text)   # process measurement unit like 1$ to one dollar, etc.
    text = ''.join(char for char in unicodedata.normalize('NFD', text)  # NFD mean Normalization Form Decomposed
                    if unicodedata.category(char) != 'Mn')  # strip accents char
    text = re.sub("[^ A-Za-z'.,?!\-]", "", text)     # remove all non-alphanumeric char
